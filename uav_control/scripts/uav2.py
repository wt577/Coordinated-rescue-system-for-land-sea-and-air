...
class UGVFollowerPureGPS:
    def __init__(self):
        ...
        # 订阅无人机发布的救援位置
        self.target_pos_sub = rospy.Subscriber("/uav1/position1", String, self.target_callback)
        ...
        self.arrival_pub = rospy.Publisher("/uav2/arrived", String, queue_size=10)
        
        # 服务代理
        rospy.loginfo("等待MAVROS服务...")
        ...
        rospy.loginfo("无人车(UGV)纯GPS控制器启动，等待指令...")
    ...
    def global_pos_callback(self, msg):
        ...
        if msg.status.status >= NavSatStatus.STATUS_FIX:
            ...
            if not self.start_recorded:
                ...
                rospy.loginfo(f"无人车起点已记录: 纬度={msg.latitude:.8f}, 经度={msg.longitude:.8f}")
        else:
            self.has_gps_fix = False
    
    def target_callback(self, msg):
        """接收并解析无人机发来的GPS坐标或返回起点指令"""
        rospy.loginfo(f"接收到指令: '{msg.data}'")
        ...
        if msg.data == "back_to_start":
            if self.start_gps:
                ...
                self.reached_target = False
                rospy.loginfo(f"无人车设置返回起点: 纬度={self.start_gps['lat']:.8f}, 经度={self.start_gps['lon']:.8f}")
            else:
                rospy.logwarn("无人车起点坐标未记录，无法返回起点")
            return
        ...
        try:
            ...
            rospy.loginfo(f"无人车已设置GPS目标点: 纬度={lat:.8f}, 经度={lon:.8f}")
        except (ValueError, IndexError) as e:
            rospy.logwarn(f"无人车GPS坐标解析失败: {e}，忽略该指令。")
    ...
    def run(self):
        """主循环 - 纯GPS导航的最终稳定流程"""
        rospy.loginfo("等待有效的GPS定位...")
        while not rospy.is_shutdown() and not self.has_gps_fix:
            rospy.loginfo_throttle(5, "等待GPS信号...")
            self.loop_rate.sleep()
        
        rospy.loginfo("GPS定位已就绪。您现在可以随时切换到OFFBOARD模式。")
        
        while not rospy.is_shutdown():
            #自动解锁
            if self.current_state.mode == "OFFBOARD" and not self.current_state.armed:
                rospy.logwarn_throttle(2, "在OFFBOARD模式下但未解锁，尝试自动解锁...")
                try:
                    self.arm_srv(True)
                except rospy.ServiceException as e:
                    rospy.logerr_throttle(2, f"解锁服务调用失败: {e}")
            ...
            if setpoint_to_send is None:
                if self.has_gps_fix:
                    ...
                    if self.current_state.mode != "OFFBOARD":
                        rospy.loginfo_throttle(5, f"非OFFBOARD模式 (当前: {self.current_state.mode})。发送心跳，等待切换...")
                    else:
                        rospy.loginfo_throttle(5, "OFFBOARD模式已激活，发送心跳，等待目标指令...")
                else:
                    rospy.logwarn_throttle(5, "GPS信号丢失，无法发送设定点！")
